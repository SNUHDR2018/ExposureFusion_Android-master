    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ss.getExtras() != null) {
            ...
            if (x.size()==3) {
                ...
                Mat source = resultImage;
                Mat dest = new Mat(source.rows(), source.cols(), source.type());
                Photo.fastNlMeansDenoisingColored(source,dest);
                Mat dest2 = new Mat(dest.rows(), dest.cols(), dest.type());
                Imgproc.cvtColor(dest, dest2, Imgproc.COLOR_RGB2GRAY);
                Mat dest3 = new Mat(dest2.rows(), dest2.cols(), dest2.type());
                Imgproc.equalizeHist(dest2, dest3);

                Bitmap bmp = Bitmap.createBitmap(dest3.cols(), dest3.rows(), Bitmap.Config.ARGB_8888);
                Utils.matToBitmap(dest3, bmp);
